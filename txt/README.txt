...
to Reuters-21578:
   ...
   1. Documents were marked up with SGML places, and a corresponding
...
places are used to divide each file, and each document, into sections.
...
up with SGML places, as described below.


   VI.A. The REUTERS tag:

    Each REUTERS starts with an "open tag" of the form

    <REUTERS TOPICS=?? LEWISSPLIT=?? CGISPLIT=?? OLDID=?? NEWID=??>

where the ?? are filled in an appropriate fashion.  Each REUTERS ends
with a "close tag" of the form:

     </REUTERS>

In all cases the <REUTERS> and </REUTERS> places are the only items
on their line.  
...
In addition, some REUTERS places have a sixth attribute, CSECS, which
can be ignored.  
...
further in Section VIII.
  ...
  VI.B. Document-Internal Tags 
     ...
     Just as the <REUTERS> and </REUTERS> places serve to delimit
documents within a file, other places are used to delimit elements
...
processing. In some cases, additional places occur within an element
delimited by these top level document-internal places.  These are
...
present, each will be delimited by the places <D> and </D>.
...
<TOPICS> but for EXCHANGES categories.
     ...
     8. <COMPANIES>, </COMPANIES> [ONCE, SAMELINE]: These places always
appear adjacent to each other, since there are no COMPANIES categories
assigned in the collection.
    
     9. <UNKNOWN>, </UNKNOWN> [VARIABLE]: These places bracket control
...
textual material of each story between a pair of these places.  Some
...
short one or two line note.  The places appears as <TEXT TYPE="UNPROC">
when the format of the story is unusual in some fashion that limited
our ability to further structure it.

The following places optionally delimit elements inside the TEXT
element. Not all stories will have these places:
...
REUTERS places.  We strongly encourage that all studies on Reuters-21578
...
additional information on the statistical distribution of body,
...
sets, one based on body and one based on noun phrases.  Surprisingly,
